...
function InnerPlayerDrawer({
  ...
  const scoreSortedPlayers: Player[] = [...players].filter(p => p.guid !== currentPlayer.guid).sort(sortPlayer);

  const generateNormalPlayerCoin = (player: Player, isBig: boolean = false): JSX.Element => {
    const lastPlayerGuess: IMessage | undefined = userGuesses.slice(0).reverse().find(guess => guess.player === player);

    return (
      <div className={s.playerCoin}>
        {
  ...
  return (
    <div className={s.container}>
      <div className={s.playerCoinContainer} onClick={toggleLog}>
        ...
        <Hidden xsDown>
          {
            scoreSortedPlayers.map(p => generateNormalPlayerCoin(p))
          }
        </Hidden>
